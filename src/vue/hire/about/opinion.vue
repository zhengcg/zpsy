		...
		<div class="reportBox" v-if="submitStatus">
			<div class="typeBox">
				...
				<div class="typeConfirm">
					<div @click="checkType(1)" v-bind:class="{active:typeCheck==1}">产品问题</div>
					<div @click="checkType(2)" v-bind:class="{active:typeCheck==2}">扫码问题</div>
					<div @click="checkType(3) " v-bind:class="{active:typeCheck==3}">意见建议</div>
					<div @click="checkType(4) " v-bind:class="{active:typeCheck==4}">其他</div>
				</div>
			</div>
		...
		data(){
			return{
				...
				api:{
					...
					creatReport:'/genuine/feedback/addFeedback'
		...
		          }).then(function(res){
			...
			init:function(){
				...
				_this.uploader = Qiniu.uploader({
				    ...
				    domain: this.qinniuToken.domain
,     // bucket域名，下载资源时用到，必需
			                ...
			                for (var i = 0, len = files.length; i < len; i++) {
		                        !function (i) {
		                            _this.previewImage(files[i], function (imgsrc) {
		                                $('#container').prepend('<div class="introImg fl"><i data-value='+ files[i].id +'>x</i><img class="introIcon" src="'+ imgsrc +'" /></div>');
		                            })
...
//				})
		    ...
		    checkType:function(typeNumber){
		    	this.typeCheck = typeNumber;
		    },
		    formClick:function(){
	    		var _this = this;
